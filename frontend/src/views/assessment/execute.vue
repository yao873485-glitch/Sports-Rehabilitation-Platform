<template>
  <div class="app-container">
    ...
    <el-card style="margin-top: 20px;">
      <div style="overflow-x: auto;">
        ...
        >
          ...
          <el-table-column type="index" label="序号" width="80" align="center" :index="1" />
          <el-table-column prop="executionType" label="执行场景" align="center" width="150" />
          <el-table-column prop="scaleName" label="量表名称" align="center" width="250" show-overflow-tooltip />
          <el-table-column prop="executionFrequency" label="执行频率" align="center" width="150" />
          <el-table-column label="周期（周）" align="center" width="120">
            <template slot-scope="scope">
              {{ scope.row.cycleDuration || 0 }}
            </template>
          </el-table-column>
          <el-table-column label="总次数（次）" align="center" width="120">
            <template slot-scope="scope">
          ...
          <el-table-column label="操作" align="center" width="180" fixed="right">
            <template slot-scope="scope">
              <el-button
                size="mini"
                type="text"
                @click.stop="handleExecuteItem(scope.row)"
              >
                执行
              </el-button>
              <el-button
                size="mini"
                type="text"
                @click.stop="handleListItem(scope.row)"
              >
                清单
              </el-button>
            </template>
    ...
    <execution-dialog
      ...
      :assessment-plan-id="currentAssessmentPlanId"
      @success="getDetail(assessmentDetail.id)"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    /** 执行评定项目 */
    handleExecuteItem(row) {
      // 打开执行弹窗
      this.currentAssessmentPlanId = row.id
...
.el-table {
  ...
  // 选中行样式 - 勾选复选框后的背景色
  ::v-deep .el-table__body tr.el-table__row.current-row {
