...
public class ContentApplicationConfigController {
    ...
    @Autowired
    private ContentConfigOperationLogMapper logMapper;
    ...
    @ApiOperation("获取配置操作日志")
    @GetMapping("/logs")
    public Result<List<ContentConfigOperationLog>> getLogs(
            @RequestParam String contentId,
            @RequestParam String contentType) {
        List<ContentConfigOperationLog> logs = logMapper.selectByContentIdAndType(contentId, contentType);
