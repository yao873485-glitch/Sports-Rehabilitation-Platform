...
public class EducationImageContentController {
    ...
    /**
     * 根据ID获取图文内容详情(支持contentId字符串或数字ID)
     */
    @Operation(summary = "根据ID获取图文内容详情")
    @GetMapping("/{id}")
    public Result<EducationImageContent> getContentById(@PathVariable String id) {
        log.info("获取图文内容详情，ID：{}", id);
        try {
            EducationImageContent content = null;

            // 尝试按数字ID查询
            try {
                Long numericId = Long.parseLong(id);
                content = contentService.getById(numericId);
            } catch (NumberFormatException e) {
                // 如果不是数字,按contentId查询
                content = contentService.lambdaQuery()
                        .eq(EducationImageContent::getContentId, id)
                        .one();
            }
