...
public class EduVideoAssetController {
    ...
    private final EduVideoAssetService eduVideoAssetService;
    ...
    @Operation(summary = "根据ID获取视频素材详情")
    @GetMapping("/{id}")
    public Result<EduVideoVO> getEduVideoById(@PathVariable Long id) {
        EduVideoVO videoVO = eduVideoAssetService.getEduVideoById(id);
        if (videoVO == null) {
