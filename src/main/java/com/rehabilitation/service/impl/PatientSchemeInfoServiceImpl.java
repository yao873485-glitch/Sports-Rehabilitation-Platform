...
public class PatientSchemeInfoServiceImpl implements PatientSchemeInfoService {
    ...
    private PatientSchemeInfoVO convertToVO(PatientSchemeInfo patientSchemeInfo) {
        ...
        // 从patient表获取详细信息
        PatientInfo patientInfo = patientInfoMapper.selectById(patientSchemeInfo.getPatientId());
        if (patientInfo != null) {
            vo.setPatientName(patientInfo.getName());
            vo.setGenderDesc(patientInfo.getGender());
            vo.setMedicalRecordNo(patientInfo.getMedicalRecordNo());
            vo.setDiseaseType(patientInfo.getDiseaseType());
            vo.setOrgName(patientInfo.getEnrollmentInstitution());

            // 计算年龄
            if (patientInfo.getBirthDate() != null) {
                vo.setAge(Period.between(patientInfo.getBirthDate(), LocalDate.now()).getYears());
            }
        }
